...
const controllerTimeoutMs = 20000;
...
const buildRefusalResponse = (transcript: string) => ({
  statusCode: 200,
...
export const handler: Handler = async (event) => {
  ...
  try {
    payload = parseBody<VoiceChatRequest>(event.body) || {};
  } catch (error) {
    return {
      statusCode: 400,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
  ...
  try {
    ...
    if (!transcript) {
      return {
        statusCode: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        body: JSON.stringify({ error: 'Empty transcript' }),
      };
    }

    if (isBlockedHealthRequest(transcript)) {
      return buildRefusalResponse(transcript);
    }
    ...
    if (!chatResponse.ok) {
      ...
      return { statusCode: chatResponse.status, headers: corsHeaders, body: text };
    }
    ...
    if (!ttsResponse.ok) {
      ...
      return { statusCode: ttsResponse.status, headers: corsHeaders, body: text };
    }
